...
int main ()
{
  ...
  //PID::Init(double Kpi, double Kii, double Kdi, double output_lim_maxi, double output_lim_mini)
  //@student:  create pid (pid_steer) for steer command and initialize values
  //The output of the STEER controller should be inside [-1.2, 1.2].
          
  PID pid_steer = PID();
  pid_steer.Init(0.32, 0.003, 0.3, 1.2, -1.2);


  // initialize pid throttle
  //The output of the THROTTLE controller should be inside [-1, 1].
          
  //@student: create pid (pid_throttle) for throttle command and initialize values
  ...
  h.onMessage([&pid_steer, &pid_throttle, &new_delta_time, &timer, &prev_timer, &i, &prev_timer](uWS::WebSocket<uWS::SERVER> ws, char *data, size_t length, uWS::OpCode opCode)
  {
        ...
        if (s != "") {
          ...
          //int closest_index = find_closest_point(x_position, y_position, x_points, y_points);
          ...
          // Throttle error is velocity difference between current speed (velocity) and desired speed (last element in v_points)
          
          //The last point of v_points vector contains the velocity computed by the path planner which is our REFERENCE
