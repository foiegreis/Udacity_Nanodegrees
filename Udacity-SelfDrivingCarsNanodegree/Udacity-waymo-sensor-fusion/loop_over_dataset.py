...
## EXEC settings - Selective execution and visualization
...
if ex =='ID_S1_EX1':
...
elif ex in ['ID_S2_EX1', 'ID_S2_EX2', 'ID_S2_EX3']:
    data_filename='training_segment-1005081002024129653_5313_150_5333_150_with_camera_labels.tfrecord'
...
elif ex in ['ID_S3_EX1', 'ID_S3_EX2'] :
    data_filename='training_segment-1005081002024129653_5313_150_5333_150_with_camera_labels.tfrecord'
    show_only_frames = [50, 60] #[50, 51]
    exec_detection = ['bev_from_pcl', 'detect_objects']
    exec_tracking = []
    exec_visualization = ['show_objects_in_bev_labels_in_camera']
    configs_det = det.load_configs(model_name='fpn_resnet')

elif ex in ['ID_S4_EX1', 'ID_S4_EX2']:
    data_filename = 'training_segment-1005081002024129653_5313_150_5333_150_with_camera_labels.tfrecord'
    ...
    exec_visualization = ['show_detection_performance']
    configs_det = det.load_configs(model_name="darknet")
    #configs_det.use_labels_as_objects = True for testing using the ground truth labels
...
while True:
    try:
        ...
        if 'show_range_image' in exec_list:
            img_range, img_rangeFRONT, img_rangeLEFT = pcl.show_range_image(frame, lidar_name)
            img_range = img_range.astype(np.uint8)
            img_rangeFRONT = img_rangeFRONT.astype(np.uint8)
            img_rangeLEFT = img_rangeLEFT.astype(np.uint8)

            cv2.imshow('range_image', img_range)
            cv2.imshow('range_image FRONT', img_rangeFRONT)
            cv2.imshow('range_image LEFT', img_rangeLEFT)
