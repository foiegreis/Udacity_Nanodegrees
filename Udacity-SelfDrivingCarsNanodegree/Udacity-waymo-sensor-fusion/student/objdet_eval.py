...
def measure_detection_performance(detections, labels, labels_valid, min_iou=0.5):
    ...
    for label, valid in zip(labels, labels_valid):
        ...
        if valid: # exclude all labels from statistics which are not considered valid
            ...
            for obj in detections:
                ...
                obj_id, x_coor, y_coor, z_coor, h_coor, w_obj, l_obj, yaw_obj =obj
    ...
    det_results = [all_positives, true_positives, false_negatives, false_positives]
    det_performance = [ious, center_devs, det_results]
    print('Results- all_positives: {}, true_positives: {}, false_negatives: {}, false_positives: {} '.format(all_positives, true_positives, false_negatives, false_positives))
...
def compute_performance_stats(det_performance_all):
    ...
    false_negatives = 0
    false_positives = 0
    for pos in det_results:
        positives += pos[0]
        true_positives += pos[1]
        false_negatives += pos[2]
        false_positives += pos[3]
    ...
    #I've been running the project on my m1 mac, via terminal.
    #At this point, this error comes out
    
    ##This program needs access to the screen. Please run with a
    ##Framework build of python, and only when you are logged in
    ##on the main display of your Mac.
    
    #A workaround to display the metrics will be for me to save the values in a json file and then
    #plot the charts in the pycharm ide. So the following lines have been added in order to perform this. You'll find the module display_charts.py in the /student folder
    ...
    #-------------------------------------------------------------
    results_out = {'precision': precision,
               'recall': recall,
               'ious': ious_all,
               'devs_x': devs_x_all,
               'devs_y': devs_y_all,
    ...
    with open("/student/data.json", "w") as res_file:
        json.dump(results_out, res_file)
