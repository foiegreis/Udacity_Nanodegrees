...
double BehaviorPlannerFSM::get_look_ahead_distance(const State& ego_state) {
  ...
  //@student
  //it will be velocity_mag^2 / 2* accel_mag
...
State BehaviorPlannerFSM::state_transition(const State& ego_state, State goal,
  ...
  if (_active_maneuver == FOLLOW_LANE) {
    ...
    if (is_goal_in_junction) {
      ...
      //@student
      //we project stop_line_buffer on x and y axes
      ...
      //@student
      //the goal speed should be 0.0 in all axes because we're stopping
      goal.velocity.x = 0.0;  
      goal.velocity.y = 0.0;  
      goal.velocity.z = 0.0;  
      ...
      //@student
      //we'll project the speed to the axes by sin and cos of yaw
